...
const UPLOAD_FIELDS: { key: UploadField; label: string; hint: string; formField: keyof KycValues }[] = [
  {
    key: 'id_front',
...
function StatusBanner({ status }: { status: string }) {
  ...
  return (
    <div className={cn('flex items-start gap-3 rounded-sm border px-4 py-3 text-sm mb-6', config.cls)}>
      {config.icon}
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <FormField control={form.control} name="full_name" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input placeholder="As shown on your Emirates ID" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                <FormField control={form.control} name="nationality" render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl><Input placeholder="United Arab Emirates" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )} />
                    ...
                    <FormField
                      ...
                      render={() => (
                        <FormItem>
                          ...
                          <label
                            className={cn(
                              'flex flex-col items-center justify-center gap-2 rounded-sm border-2 border-dashed px-4 py-6 text-center cursor-pointer transition-colors',
                              uploaded
            ...
            <section>
              <div className="rounded-sm border border-border bg-card p-4">
                <FormField control={form.control} name="consent_identity_verification" render={({ field }) => (
                  <FormItem className="flex items-start gap-3 space-y-0">
                    ...
                    <div>
                      <FormLabel className="font-normal text-sm leading-snug">
                        I consent to Bayty Technologies collecting and processing the above personal data and identity documents for the purpose of identity verification, in accordance with the UAE Federal Decree-Law No. 45 of 2021 on Personal Data Protection and Saudi PDPL.
                      </FormLabel>
                      <FormMessage />
                    </div>
                  </FormItem>
            ...
            <div className="flex justify-end">
              <Button type="submit" disabled={form.formState.isSubmitting} className="min-w-[160px]">
                {form.formState.isSubmitting ? (
                  <><Loader2 size={14} className="me-2 animate-spin" />Submitting…</>
                ) : (
                  <><Upload size={14} className="me-2" />Submit for Verification</>
                )}
              </Button>
            </div>
          </form>
