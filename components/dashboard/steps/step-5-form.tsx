      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        {p2 && (
          <section>
            ...
            <div className="rounded-sm border border-border bg-card px-4">
              ...
              {p2.country && <SummaryRow label="Location" value={`${p2.city ?? ''}, ${p2.country}`} />}
              {p2.project_type && <SummaryRow label="Type" value={p2.project_type.replace(/_/g, ' ')} />}
              {p2.project_phase && <SummaryRow label="Phase" value={p2.project_phase.replace(/_/g, ' ')} />}
              {p2.capex_band && <SummaryRow label="CAPEX Band" value={p2.capex_band.replace(/_/g, ' ')} />}
              {p2.funding_source && <SummaryRow label="Funding" value={p2.funding_source.replace(/_/g, ' ')} />}
            </div>
          </section>
        )}
        ...
        <section>
          <FormField control={form.control} name="submission_notes" render={({ field }) => (
            <FormItem>
              <FormLabel>Message to Bayty Team <span className="text-muted-foreground">(optional)</span></FormLabel>
              <FormControl>
                <Textarea
        ...
        <section>
          ...
          <div className="space-y-4 rounded-sm border border-border bg-card p-4">
            ...
            <FormField control={form.control} name="confirm_data_accuracy" render={({ field }) => (
            ...
            <FormField control={form.control} name="confirm_authority_to_submit" render={({ field }) => (
              <FormItem className="flex items-start gap-3 space-y-0">
                <FormControl>
                  <Checkbox
                    checked={field.value === true}
                ...
                <div>
                  <FormLabel className="font-normal text-sm leading-snug">
                    I am authorized by my organization to submit this project intake request on their behalf.
                  </FormLabel>
                  <FormMessage />
                </div>
              </FormItem>
            ...
            <FormField control={form.control} name="consent_data_processing" render={({ field }) => (
              <FormItem className="flex items-start gap-3 space-y-0">
                <FormControl>
                  <Checkbox
                    checked={field.value === true}
                ...
                <div>
                  <FormLabel className="font-normal text-sm leading-snug">
                    I consent to Bayty Technologies processing the submitted data for the purpose of evaluating and delivering the requested services.
                  </FormLabel>
                  <FormMessage />
                </div>
              </FormItem>
            ...
            <FormField control={form.control} name="consent_pdpl" render={({ field }) => (
              <FormItem className="flex items-start gap-3 space-y-0">
                <FormControl>
                  <Checkbox
                    checked={field.value === true}
                ...
                <div>
                  <FormLabel className="font-normal text-sm leading-snug">
                    ...
                    <span className="text-primary">Saudi Personal Data Protection Law (PDPL)</span> and{' '}
                    <span className="text-primary">UAE Federal Decree-Law No. 45 of 2021</span>, and that my organization accepts the applicable data protection obligations.
                  </FormLabel>
                  <FormMessage />
                </div>
              </FormItem>
