  ...
  return (
    <div className="space-y-6">
      <section>
        ...
        >
          <Upload size={28} className="text-muted-foreground/60 mb-2" />
          <p className="text-muted-foreground text-sm">
            Drag & drop a file here, or <span className="text-primary">click to browse</span>
          </p>
          <p className="text-muted-foreground/60 mt-1 text-xs">
            Will be uploaded as:{' '}
            <strong className="text-foreground">{DOC_TYPE_LABELS[selectedType]}</strong>
          </p>
        </div>
        ...
        <section>
          ...
          <ul className="space-y-2">
            {documents.map((doc) => (
              <li
                key={doc.id}
                className="border-border bg-card flex items-center gap-3 rounded-sm border px-4 py-3"
              >
