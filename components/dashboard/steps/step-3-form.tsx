  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        {/* Service modules */}
        <section>
          <h2 className="mb-1 text-sm font-semibold uppercase tracking-wider text-muted-foreground">Service Modules Required</h2>
          <p className="mb-4 text-xs text-muted-foreground">Select all modules your project needs.</p>
          <FormField
        ...
        {/* Users & volumes */}
        <section>
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-wider text-muted-foreground">Scale & Volumes</h2>
          <div className="grid gap-4 sm:grid-cols-2">
            <FormField control={form.control} name="expected_users" render={({ field }) => (
              <FormItem>
                <FormLabel>Expected Platform Users <span className="text-muted-foreground">(optional)</span></FormLabel>
                <FormControl><Input type="number" min="1" placeholder="50" {...field} onChange={(e) => field.onChange(e.target.value)} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />
            <FormField control={form.control} name="expected_doc_volume" render={({ field }) => (
              <FormItem>
                <FormLabel>Expected Monthly Documents <span className="text-muted-foreground">(optional)</span></FormLabel>
                <FormControl><Input type="number" min="0" placeholder="500" {...field} onChange={(e) => field.onChange(e.target.value)} /></FormControl>
                <FormDescription className="text-xs">Estimate for document control sizing.</FormDescription>
                <FormMessage />
              </FormItem>
            )} />
          </div>
        ...
        {/* Integrations */}
        <section>
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-wider text-muted-foreground">Existing Systems / Integrations</h2>
          <FormField
            control={form.control}
            name="integrations"
            render={() => (
        ...
        {/* Preferences */}
        <section>
          <h2 className="mb-4 text-sm font-semibold uppercase tracking-wider text-muted-foreground">Platform Preferences</h2>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <FormField control={form.control} name="data_residency" render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  ...
                  <SelectContent>
                    {DATA_RESIDENCY_OPTIONS.map((v) => (
                      <SelectItem key={v} value={v}>{DATA_RESIDENCY_LABELS[v]}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                <FormMessage />
            ...
            <FormField control={form.control} name="language_support" render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  ...
                  <SelectContent>
                    {LANGUAGE_SUPPORT_OPTIONS.map((v) => (
                      <SelectItem key={v} value={v}>{LANGUAGE_LABELS[v]}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                <FormMessage />
            ...
            <FormField control={form.control} name="target_go_live" render={({ field }) => (
              <FormItem>
                <FormLabel>Target Go-Live Date <span className="text-muted-foreground">(optional)</span></FormLabel>
